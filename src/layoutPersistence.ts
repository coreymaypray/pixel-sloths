...
export function readLayoutFromFile(): Record<string, unknown> | null {
  ...
  try {
    ...
    console.error('[Pixel Sloths] Failed to read layout file:', err);
    return null;
  }
}
...
export function writeLayoutToFile(layout: Record<string, unknown>): void {
  ...
  try {
    ...
    console.error('[Pixel Sloths] Failed to write layout file:', err);
  }
}
...
export function migrateAndLoadLayout(
  ...
  if (fromFile) {
    ...
    if (defaultRevision > fileRevision) {
      console.log(
        `[Pixel Sloths] Layout revision outdated (${fileRevision} < ${defaultRevision}), resetting to bundled default`,
      );
      writeLayoutToFile(defaultLayout!);
      return { layout: defaultLayout!, wasReset: true };
    }
    console.log('[Pixel Sloths] Layout loaded from file');
  ...
  if (fromState) {
    console.log('[Pixel Sloths] Migrating layout from workspace state to file');
  ...
  if (defaultLayout) {
    console.log('[Pixel Sloths] Writing bundled default layout to file');
    writeLayoutToFile(defaultLayout);
...
/**
 * Watch ~/.pixel-sloths/layout.json for external changes (other VS Code windows).
...
export function watchLayoutFile(
  ...
  function checkForChange(): void {
    ...
    try {
      ...
      console.log('[Pixel Sloths] External layout change detected');
      onExternalChange(layout);
    } catch (err) {
      console.error('[Pixel Sloths] Error checking layout file:', err);
    }
  }
